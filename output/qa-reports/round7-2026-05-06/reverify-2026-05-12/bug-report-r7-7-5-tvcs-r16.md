...
Phát hiện **9** lỗi có SRS reference cụ thể trong R7.7.5 deep review (R16 + R17 unblock + R20 TV-041). Hiện trạng (sau R19b UI clean retest 2026-05-12): **1 Open** (BUG-005 button Công khai/Hủy công khai chưa wire PARTIAL) · **8 Closed** sau retest R17/R19/R19b/R20. R19b clean UI confirmed BUG-001 FIXED (R19 API probe false-negative).
...
| ~~BUG-BE-TVCS-R16-001~~ | Major | P1 | Workflow | TV-023, TV-024, TV-025, TV-043 | `srs-update-2026-5-5/srs-fr-12-tv-chuyen-sau.md §Tư liệu pháp luật + UC TLPL` | ~~TLPL VV CRUD endpoint chưa expose — block toàn bộ luồng quản lý tư liệu pháp luật gắn TVCS~~ | Closed ✅ R19b |
...
## ~~BUG-BE-TVCS-R16-001~~ — TLPL VV CRUD endpoint chưa expose [CLOSED ✅ R19b]

> **Re-test:** 2026-05-12 19:00:00 R19b — ✅ PASS (Closed-verified). **UI clean test khẳng định FIXED hoàn toàn** — R19 trước probe API 11 path → tất cả 404 nhưng false-negative do tester đoán sai path. R19b dùng UI thuần (account `cb_nv_tw_06`, isolatedContext `reverify_cbnvtw06_2026_05_12`, TVCS-20260507-0013 state DANG_TU_VAN):
> 1. Expand accordion "Tư liệu pháp luật" → render button **`Thêm tư liệu`** + empty state "Chưa có tư liệu pháp luật đính kèm." ✅
> 2. Click `Thêm tư liệu` → modal "Thêm tư liệu pháp luật" mở với form đầy đủ: Tên tư liệu * + Loại tư liệu * (5 options: Văn bản pháp luật / Tài liệu / Nghiên cứu / Tiền lệ / Khác) + Lĩnh vực pháp luật + Mô tả + File đính kèm (max 10 file, .doc/.docx/.xls/.xlsx/.pdf/.jpg/.png/.gif, ≤20MB) ✅
> 3. Fill `Tên = TLPL R19b retest BUG-TVCS-R16-001`, `Loại = Văn bản pháp luật`, `Mô tả = R19b UI retest verify endpoint hoat dong` → click `Thêm` → modal đóng, table TLPL render 1 row mới (`Trạng thái = Nháp`), action `Sửa/Công khai/Xóa` ✅
> 4. Endpoint CREATE TLPL HOẠT ĐỘNG (BE đã expose path). Evidence: [`image/r19b-tvcs-r16-001-ui-tlpl-create-success-2026-05-12.png`](image/r19b-tvcs-r16-001-ui-tlpl-create-success-2026-05-12.png).
>
> **Lesson learned:** R19 probe API 11 path bỏ qua endpoint thực dev exposed. UI test đầu tiên + API probe sau sẽ tránh false-negative này.
