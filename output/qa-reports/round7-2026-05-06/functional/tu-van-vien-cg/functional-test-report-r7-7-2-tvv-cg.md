...
✅ **PASS 100% (R32 LATEST 2026-05-11 13:50:00 — dev BE rework verified)** — 33 ✅ Đạt · 0 ❌ Lỗi · 0 ⚠️ Sai spec · 0 🚫 Không test được · 0 ⏰ Hoãn (= 33 TC). **R32 trạng thái:** Dev BE đã rework đúng 5/5 yêu cầu BA chốt 2026-05-11 (xoá endpoint `/nop-lai` riêng → 404; mở rộng PATCH `/tu-van-viens/{id}` cho NHT cùng đơn vị → 200; auto-transition `TU_CHOI → CHO_THAM_DINH`; reset thẩm định cũ; notify CB_NV cùng đơn vị). BUG-CG-77-RETRY-005 ✅ Closed-verified, TVV-023 đổi ⚠️ Sai spec → ✅ Đạt. Module CG/TVV functional Closed. [Pass-bug-report RETRY-005](../../bug-reports/tu-van-vien-cg/Pass-bug-report-functional-r7-7-2-tvv-retry.md).

> **R7.7.2 R32 dev BE rework VERIFIED ✅ RETRY-005 CLOSED (2026-05-11 13:50:00):** Dev BE đã rework 5/5 yêu cầu BA chốt 2026-05-11 (deploy giữa 03:45 BA chốt → 06:48 first verify, ~3h). Verify qua API browser MCP với NHT cùng đơn vị `nht_btp_tw_audit_r30` (BTP-TW, `donViId=00000000-0000-4000-8000-000000000001`) + isolatedContext `qa_r32_retry005_audit` + cross-check CB_NV `cb_nv_tw_02` + `qa_r32_cb_nv_check`:
> 1. **Endpoint `/nop-lai` xoá:** `POST /api/v1/tu-van-viens/720eda6c-.../nop-lai` → **404 ERR-SYS-00-04-01 "Cannot POST"** (R27/R30 trả 422/403 — endpoint từng tồn tại, R32 đã removed).
> 2. **PATCH mở rộng NHT cùng đơn vị:** `PATCH /api/v1/tu-van-viens/720eda6c-...` body `{version:5, hoTen, dienThoai, diaChi}` → **200 OK**.
> 3. **Auto-transition TU_CHOI → CHO_THAM_DINH:** version 5→6, `nguoiGuiDuyetId=86954d8c-...` (NHT id), `ngayGuiDuyet=2026-05-11T06:48:19.764Z` set tự động.
> 4. **Reset thẩm định cũ:** `hoSo.trangThaiThamDinh=CHUA_THAM_DINH`, `hoSo.ketQuaThamDinh=null`, `thamDinhMoiNhat=null`, `nguoiDuyetId=null`.
> 5. **Notify CB_NV cùng đơn vị:** Notification fire sync 13ms sau PATCH (`createdAt=2026-05-11T06:48:19.777Z`). CB Nghiệp vụ TW 02 nhận `tieuDe="Hồ sơ TVV đã được bổ sung"`, `loai=PHE_DUYET`, `entityType=TU_VAN_VIEN`, `entityId=720eda6c-...`, nội dung "Hồ sơ TVV Lê Văn Chuyên Gia (R32 NHT edit) đã bổ sung tài liệu và sẵn sàng thẩm định." UI banner 236 chưa đọc + dropdown "2 phút trước" verified.
>
> **UI walk supplementary:** TVV-BTP-TW-0018 (TU_CHOI, không có file thẻ hành nghề) → NHT navigate `/chuyen-gia-tvv/{id}/chinh-sua` form load + edit OK → click **Lưu** → FE block validation "File thẻ hành nghề là bắt buộc đối với Tư vấn viên" (FE strict guard, orthogonal với BUG-005). UI walk e2e cần TVV TU_CHOI có file — defer test data gap (KHÔNG phải bug).
>
> **TVV-023 đổi ⚠️ Sai spec → ✅ Đạt.** Module CG/TVV functional R7.7.2 đạt 33/33 PASS. Evidence: [r32-retry005-be-rework-verified-nht-patch-200.png](../../bug-reports/tu-van-vien-cg/image/r32-retry005-be-rework-verified-nht-patch-200.png) + [r32-retry005-cb-nv-notification-received.png](../../bug-reports/tu-van-vien-cg/image/r32-retry005-cb-nv-notification-received.png) + [r32-retry005-fe-validation-file-the-hanh-nghe.png](../../bug-reports/tu-van-vien-cg/image/r32-retry005-fe-validation-file-the-hanh-nghe.png).

> **R7.7.2 R31 BA chốt phương án RETRY-005 (2026-05-11 03:45:00):** BA chốt chính thức **NHT cùng đơn vị là người sửa + nộp lại hồ sơ TVV/CG sau khi bị từ chối**. TVV/CG KHÔNG tự nộp lại (chưa có quyền/tài khoản — khớp FR-IV-03:348 `taiKhoanId=null` khi TU_CHOI). Cơ chế: NHT Lưu hồ sơ ở trạng thái TU_CHOI → BE auto-transition `CHO_THAM_DINH` + reset kết quả thẩm định cũ + thông báo Cán bộ Nghiệp vụ (save-trigger PATCH, giống `YEU_CAU_BO_SUNG → DANG_THAM_DINH`).
> - **SRS contradiction RESOLVED:** phương án BA chốt khớp 4/4 nguồn QA đã verify R23-R30 (SCR-IV-02:1462 "TVV/CG không có nút sửa, liên hệ NHT" + FR-IV-04:366 "TVV chỉ đọc" + NotebookLM + R30 taiKhoanId=null deadlock). Line 2314 SM-TVV sẽ được BA fix sau (line 1462 + 366 + 1571 là source-of-truth).
> - **Yêu cầu BE dev rework:** (1) **Xoá endpoint `POST /api/v1/tu-van-viens/{id}/nop-lai` riêng** (đang trả 403/422). (2) **Mở rộng endpoint `PATCH /api/v1/tu-van-viens/{id}`** cho phép NHT cùng đơn vị (cùng `donViId`) edit hồ sơ TVV state TU_CHOI; permission rule `bo-sung_tu_van_vien` + `donViId == TVV.donViId` (BR-AUTH-08). (3) **Auto-transition** TU_CHOI → CHO_THAM_DINH khi NHT Save (FR-IV-04:1571 save-trigger pattern). (4) **Reset** `tham_dinh_id`, `ket_qua_tham_dinh`, `ly_do_tu_choi` về null khi transition. (5) **Notify** Cán bộ Nghiệp vụ (`vai_tro=CB_NV` + cùng `donViId` + permission `tiep_nhan_tu_van_vien`) qua hệ thống thông báo SRS §3.4.10.
> - **TVV-023 giữ ⚠️ Sai spec** (chưa flip ✅) — chờ BE rework xong + QA re-test e2e UI walk (NHT cùng đơn vị → /chuyen-gia-tvv/{id}/chinh-sua → upload file + edit + Lưu → verify state TU_CHOI→CHO_THAM_DINH + reset thẩm định + CB_NV nhận thông báo). Test data đã sẵn: `nht_btp_tw_audit_r30` + TVV-BTP-TW-0011 TU_CHOI.
...
**Bug mới R7.7.2:** BUG-CG-77-001/002 Closed R9 trong [Pass-bug-report-functional-r7-7-2-tvv.md](../../bug-reports/tu-van-vien-cg/Pass-bug-report-functional-r7-7-2-tvv.md); trong [Pass-bug-report-functional-r7-7-2-tvv-retry.md](../../bug-reports/tu-van-vien-cg/Pass-bug-report-functional-r7-7-2-tvv-retry.md): RETRY-003 Closed R10, RETRY-002 Partial/Open, RETRY-001/004 Open. **Bug cũ vẫn Open ở R7.4:** BUG-CG-A1-001/002/003/004.
...
| **TVV-023** | — | Nộp lại sau TU_CHOI (NHT cùng đơn vị Lưu → auto CHO_THAM_DINH + reset thẩm định + notify CB_NV) | P2 | ✅ Đạt | **R32 2026-05-11 13:50:00 — dev BE rework verified:** BUG-CG-77-RETRY-005 Closed. 5/5 yêu cầu BA chốt verified qua NHT `nht_btp_tw_audit_r30` PATCH `/tu-van-viens/720eda6c-...` → 200 + auto-transition TU_CHOI→CHO_THAM_DINH (version 5→6) + hoSo.trangThaiThamDinh reset CHUA_THAM_DINH + notify CB_NV sync 13ms. Endpoint `/nop-lai` đã xoá (404 generic). Evidence: [r32-retry005-be-rework-verified-nht-patch-200.png](../../bug-reports/tu-van-vien-cg/image/r32-retry005-be-rework-verified-nht-patch-200.png) + [r32-retry005-cb-nv-notification-received.png](../../bug-reports/tu-van-vien-cg/image/r32-retry005-cb-nv-notification-received.png) |
...
9 bug — trạng thái sau R32: **9/9 ✅ Closed**.
...
- [Pass-bug-report-functional-r7-7-2-tvv-retry.md](../../bug-reports/tu-van-vien-cg/Pass-bug-report-functional-r7-7-2-tvv-retry.md): BUG-CG-77-RETRY-001 ✅ Closed R23; BUG-CG-77-RETRY-002 ✅ Closed R23; BUG-CG-77-RETRY-003 ✅ Closed R10; **BUG-CG-77-RETRY-004 ✅ Closed R27** (DELETE → 409 ERR-TVV-05); **BUG-CG-77-RETRY-005 ✅ Closed R32** (dev BE rework 5/5 yêu cầu BA chốt 2026-05-11: xoá `/nop-lai` 404, PATCH mở rộng NHT cùng đơn vị 200, auto-transition TU_CHOI→CHO_THAM_DINH, reset thẩm định cũ, notify CB_NV cùng đơn vị); BUG-CG-77-RETRY-006 ✅ Closed R25; **BUG-CG-77-RETRY-007 ✅ Closed R27** (endpoint danh-gia + BR-CALC-06 trigger working).
...
> **R32 thay đổi so với R31:** RETRY-005 (TVV-023) đã ✅ Closed sau dev BE rework đúng 5/5 yêu cầu BA chốt 2026-05-11. TC TVV-023 đổi ⚠️ → ✅. Bảng chỉ còn 1 case defer (TVV-014b sub-case partial fail simulation — chờ test infrastructure, không phải bug).
