...
|---|---|---|---|
| ~~BUG-HD-053-DEFAULT-IMAGE-001~~ | ~~Minor~~ | ~~Modal CR-01 thiếu button "Dùng ảnh hệ thống mặc định" — R10e 2026-05-11 09:13:00 verified fix: button có mặt cạnh upload zone, click → preview render `/default-avatar-cong-khai.png` + button toggle thành "Đổi ảnh khác". Hint accept thêm `.gif` đúng spec line 1149.~~ | Closed-verified |
...
## ~~BUG-HD-021-TABS-001~~ [CLOSED] — UI render 9 tabs vs spec 7 tabs (gộp 2 cặp state)
...
## ~~BUG-HD-022-SLA-THRESHOLD-001~~ [CLOSED] — QTHT Cấu hình SLA default Ngưỡng 2 = 90% lệch spec BR-SLA-02 (QUA_HAN > 100%)
...
## ~~BUG-HD-053-DEFAULT-IMAGE-001~~ [CLOSED] — Modal Công khai CR-01 thiếu button "Dùng ảnh hệ thống mặc định"
...
## ~~BUG-HD-016-THOIGIAN-NULL-001~~ [CLOSED] — Hủy công khai không reset `thoi_gian_dang_tai` về NULL

> **Re-test:** 2026-05-10 19:08:00 R10d — ✅ PASS Closed-verified. Login `cb_pd_tw_04` → HD-20260509-010 (DA_DUYET) → click [Công khai lên Cổng PLQG] → fill mô tả → submit. GET `/api/v1/hoi-daps/{id}` trả `trangThai=CONG_KHAI, congKhai=true, thoiGianDangTai=2026-05-10T11:47:41.668Z`. Click [Hủy công khai] → confirm popup → state về DA_DUYET. GET API verify: `trangThai=DA_DUYET, congKhai=false, thoiGianDangTai=null` ✅. BR-FLOW-09 line 1102 đã enforce — `thoi_gian_dang_tai` reset NULL khi hủy CK. Bằng chứng: ![r7-hd-016-retest-r10d-thoigian-null-fixed.png](image/r7-hd-016-retest-r10d-thoigian-null-fixed.png).
