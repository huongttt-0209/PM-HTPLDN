...
> - URL vẫn `/login`
> - Form vẫn enabled với `cb_nv_tw_02 / Secret@123` đã fill
> - **0 toast/notification/alert/inline-error** hiển thị trên UI (probe `.ant-message`, `.ant-notification`, `[role="alert"]`, `.ant-form-item-explain-error`, `.ant-alert` — toastCount=0)
> - Console error 429 logged nhưng silent UI
>
> User experience: nút Đăng nhập "stall" — không có feedback gì → user retry → đốt thêm bucket → throttle window kéo dài. Bug giữ Open. Recommend FE catch 429 → show `message.error(error.message)` từ BE response (BE đã trả message tiếng Việt user-friendly).
> Screenshot: [r10-bug-auth-otp-02b-fe-silent-on-429.png](../../screenshots/r10-bug-auth-otp-02b-fe-silent-on-429.png)
