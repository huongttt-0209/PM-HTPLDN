...
Phát hiện **14** lỗi trong quá trình test functional HĐ Tư vấn (R7.7.14). Hiện trạng sau reverify R20 2026-05-13 11:00:00: **0 Open** · **14 Closed** (R20 reverify 2026-05-13 11:00:00: BUG-037 + BUG-038 đều PASS — cell Trạng thái render "Đang thực hiện" + pagination "1-2 / 2 mục").
...
|--------|----------|----------|------|--------|-------------------|-------|--------|
| ~~BUG-HDTV-034~~ | Minor | P3 | Spec conflict | — | `srs-v3.5.md line 660 M-01` ("KHÔNG có menu riêng") + BA 2026-05-11 | ~~Route /hop-dong-tv/danh-sach render standalone list page (8 records) — BA chốt không public/menu; nếu giữ thì route ẩn có guard/redirect~~ | Closed ✅ R20 |
| ~~BUG-HDTV-037~~ | Minor | P3 | UI/UX i18n | HDTV-028 | `srs-fr-14 line 369` enum + `srs-fr-04 §3.0` Quy ước UI + `srs-fr-06 Quy tắc tương tác` + NotebookLM | ~~TVV detail tab "Lịch sử hỗ trợ" → cột "Trạng thái" trong table Hợp đồng tư vấn render raw enum `DANG_THUC_HIEN` thay vì label "Đang thực hiện" (i18n missing)~~ | Closed ✅ R20 |
| ~~BUG-HDTV-038~~ | Minor | P3 | UI/UX i18n | HDTV-028 | `srs-fr-14 line 262` + SCR-II-01/SCR-V.I-01/SCR-III-01 pagination text + NotebookLM | ~~TVV detail tab "Lịch sử hỗ trợ" → pagination text dùng từ "mặt hàng" thay vì "kết quả"/"mục" (e-commerce template leak)~~ | Closed ✅ R20 |
...
## ~~BUG-HDTV-034~~ [CLOSED] — Standalone list page `/hop-dong-tv/danh-sach` tồn tại nhưng spec v2.1 nói "KHÔNG có menu riêng" (spec conflict)

> **Re-test:** 2026-05-12 22:50:00 R20 — ✅ PASS (Closed-verified). Login `cb_nv_tw_06` → navigate URL trực tiếp `/hop-dong-tv/danh-sach` → FE auto-redirect về `/dashboard` (route guard hoạt động, không còn render standalone list). Probe BE `GET /api/v1/hop-dong-tu-vans` + `?page=1&pageSize=10` cả 2 đều trả **403 ERR-PERM-SYS-00-01 "Hợp đồng tư vấn chỉ truy cập trong ngữ cảnh vụ việc/tư vấn viên/tổ chức"**. Cả 2 surface (FE route guard + BE middleware) đã align spec v3.5 line 660 + BA chốt 2026-05-11. Đóng bug.
...
## ~~BUG-HDTV-037~~ [CLOSED] — TVV detail tab "Lịch sử hỗ trợ" → table "Hợp đồng tư vấn" hiển thị raw enum `DANG_THUC_HIEN` thay vì label "Đang thực hiện" (i18n missing)

> **Re-test:** 2026-05-13 11:00:00 R20 — ✅ PASS (Closed-verified). Login `cb_nv_tw_05` → navigate TVV-BTP-TW-0035 detail (`/chuyen-gia-tvv/978354d7-feac-4330-a750-6b8c07b46c24`) → click tab "Lịch sử hỗ trợ". Cell "Trạng thái" trong table HDTV render "Đang thực hiện" (KHÔNG raw `DANG_THUC_HIEN`). `evaluate_script` filter `.ant-table-tbody td` chứa `DANG_THUC_HIEN` → returns `["Đang thực hiện", "Đang thực hiện"]` (label-only, không có raw enum). FE i18n mapping đã apply. Evidence: [`image/r20-bug-hdtv-037-038-i18n-pass.png`](image/r20-bug-hdtv-037-038-i18n-pass.png).
...
## ~~BUG-HDTV-038~~ [CLOSED] — TVV detail tab "Lịch sử hỗ trợ" → pagination text dùng từ "mặt hàng" thay vì "kết quả"/"bản ghi"/"mục" (i18n e-commerce template leak)

> **Re-test:** 2026-05-13 11:00:00 R20 — ✅ PASS (Closed-verified). Cùng session với BUG-037: `evaluate_script` get `.ant-pagination-total-text` → returns "1-2 / 2 mục" (KHÔNG còn "mặt hàng"). E-commerce template leak đã sửa. Evidence: same screenshot [`image/r20-bug-hdtv-037-038-i18n-pass.png`](image/r20-bug-hdtv-037-038-i18n-pass.png).
