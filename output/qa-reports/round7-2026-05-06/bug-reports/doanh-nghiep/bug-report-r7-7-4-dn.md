...
| **Cập nhật mới nhất** | 2026-05-10 12:25:00 (R13 close 2 bug) |
...
File này gộp toàn bộ bug R7.7.4 Doanh nghiệp từ R7 đến R13 (trước đây tách 4 file rời, R12 consolidate). Tổng **6 bug active** (1 Open + 5 Closed) + **1 Withdrawn** (false positive). R13 dev fix → đóng FILTER-DATE-001 + MENU-ROUTE-001.
...
|--------|----------|----------|------|--------|-------------------|-------|--------|
| BUG-DN-022-ME-MISSING-LV-001 | Major | P1 | Data | DN-022 | `srs-fr-07-doanh-nghiep.md` FR-V.III-01 Inputs row 17 (`linh_vuc_ids` structured M-N) + permission matrix DOANH_NGHIEP × DN = 📝 RU* | GET `/api/v1/doanh-nghieps/me` thiếu field `linhVucIds`; PATCH /me lại accept linhVucIds → asymmetric serializer (write OK, read miss) | Open |
...
## BUG-DN-022-ME-MISSING-LV-001 — Endpoint `GET /api/v1/doanh-nghieps/me` thiếu field `linhVucIds` (asymmetric serializer — PATCH accept nhưng GET không trả)
...
| `9999999998` | DN | DP | MENU-ROUTE-001 (Open R10/R11 → Closed R13), ME-MISSING-LV-001 (Open) |

---

*Bug report consolidated: 2026-05-10 12:25:00 | huongttt + Claude (MCP chrome-devtools) — gộp từ 4 file rời (deploy-gap, dn-018-nht-perm-leak, dn-menu-route-broken, dn-002-date-filter-param-mismatch). R13 close 2 bug (FILTER-DATE-001 + MENU-ROUTE-001) sau dev fix.*
