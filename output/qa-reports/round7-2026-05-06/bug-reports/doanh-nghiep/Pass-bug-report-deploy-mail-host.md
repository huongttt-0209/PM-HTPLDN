...
> **Re-test 2026-05-09 23:35:00 R7 (qtht_03):** ✅ PASS (Closed-verified). Cả 2 bug đã FIX. Self-reg DN mới (MST `5700099991`, email `verify.mailhost.r7@test.htpldn.vn`) → email MailHog có link host `103.172.236.130:3000` (đúng deploy host) + bao trong `<a href>` clickable. Paste link gốc vào browser → API `POST /api/v1/auth/verify-email` 200 → user account chuyển trạng thái `Hoạt động` (verify qtht_03 list TK).
...
|---|---|---|---|---|---|---|---|
| ~~BUG-DEPLOY-MAIL-HOST-001~~ | Major | P1 | Workflow | R7.2.4 | `FR-VIII-22 §Processing Bước 6` + `AC dòng 2` | ~~Email kích hoạt dùng host `localhost:3000` → user click ERR_CONNECTION_REFUSED~~ | Closed |
| ~~BUG-DEPLOY-MAIL-LINK-002~~ | Major | P1 | UI/UX | R7.2.4 | `FR-VIII-22 §Processing Bước 6` ("link kích hoạt") | ~~Email body render link dạng plain text — KHÔNG có thẻ `<a>` clickable~~ | Closed |
...
## ~~BUG-DEPLOY-MAIL-HOST-001~~ [CLOSED] — Email kích hoạt dùng host `localhost:3000`

> **Re-test:** 2026-05-09 23:35:00 R7 — ✅ PASS (Closed-verified). Tài khoản `qtht_03`. Self-reg DN MST `5700099991` → MailHog email body chứa link `http://103.172.236.130:3000/auth/verify-email?token=68412f4c-31ea-4f33-b3b6-829e8e635542` (host = deploy thực, không còn `localhost`). Paste link → browser navigate `/auth/verify-email`, BE `POST /api/v1/auth/verify-email` 200, redirect `/login`. User account chuyển trạng thái `Hoạt động` trong list QTHT. Không còn `ERR_CONNECTION_REFUSED`.
...
![Email localhost link](image/deploy-mail-host-001-mailhog-email-localhost-link.png)

**2. Browser kết quả khi user paste link gốc:**

![ERR_CONNECTION_REFUSED](image/deploy-mail-host-001-err-connection-refused.png)
...
## ~~BUG-DEPLOY-MAIL-LINK-002~~ [CLOSED] — Email body render link dạng plain text, không clickable

> **Re-test:** 2026-05-09 23:35:00 R7 — ✅ PASS (Closed-verified). Tài khoản `qtht_03`. Self-reg DN MST `5700099991` → email body decoded có 1 thẻ `<a href="http://103.172.236.130:3000/auth/verify-email?token=...">...</a>` bao quanh link kích hoạt. MailHog HTML preview render link là anchor (uid `7_28` link element trong iframe `about:srcdoc`). Bằng chứng: `evaluate_script` `anchor_count: 1`, `anchors: [{attrs: 'href="..."', text: '...'}]`.
...
![Email plain text](image/deploy-mail-host-001-mailhog-email-localhost-link.png)
