...
| **Ngày** | 2026-05-13 15:30:00 |
| **Loại test** | Workflow E2E + bugfix re-verify (R20) + R19c-seed UI walk + 5 TC unblock (R19c followup) |
| **Round** | R20 |
...
Phát hiện **16** lỗi có SRS reference cụ thể trong workflow Đánh giá Hiệu quả HTPLDN (FR-08). Hiện trạng (sau R20 deep-verify SRS v3.5 2026-05-13 12:30:00): **0 Open** · **15 Closed** · **1 Invalid** (BUG-DG-016 dùng state machine v3 cũ — không phải bug, đếm gộp vào Closed).
...
|--------|----------|----------|------|--------|-------------------|-------|--------|
| ~~BUG-FUNC-DG-016~~ | Critical | P0 | Workflow / state machine | B8/B9 (FR-VI-06 Bước 8 — SRS v3.5) | `srs-update-2026-5-5/srs-fr-08-danh-gia.md:73,494,1025,1052` (SM v3.5 8 states KHÔNG có `DA_DANH_GIA`; transition `THUC_HIEN → BAO_CAO` AUTO theo FR-VI-06 Bước 8) | ~~Thiếu endpoint forward state advance từ `THUC_HIEN` — POST mọi endpoint variant đều 404~~ | ~~Closed (Not-a-bug)~~ |
| ~~BUG-FUNC-DG-014~~ | Medium | P2 | UI data display | TC11/TC12 modal Phân công (FR-VI-03) | `srs-update-2026-5-5/srs-fr-08-danh-gia.md` FR-VI-03 Inputs row 4 + line 798 (SCR-VI-01 Tab 2) | ~~Dropdown "Lĩnh vực" modal "Thêm người đánh giá" render raw UUID thay vì tên Vietnamese~~ | Closed ✅ R20 |
...
## ~~BUG-FUNC-DG-016~~ [INVALID — KHÔNG PHẢI BUG] — Thiếu endpoint forward state advance từ `THUC_HIEN` (dùng state machine v3 cũ)

> **Re-test:** 2026-05-13 15:30:00 R21 — ✅ PASS (Closed-verified). Live verify với `cb_nv_tw_01` qua GET `/api/v1/ke-hoach-danh-gias`: pool 9 đợt có distribution states `{THUC_HIEN:1, LAP_KE_HOACH:2, DANG_DANH_GIA:2, CHO_DUYET_PC:2, HOAN_THANH:2}`. Đợt `c521f1f1-...` đã advance THUC_HIEN→DANG_DANH_GIA ver 5 qua workflow chấm điểm (PUT `/ket-quas` 200 — R12 verified). UI Danh sách DG render đầy đủ tab 11 state (Lập kế hoạch / Phân công / Chờ duyệt PC / Thực hiện / Đang đánh giá / Đã đánh giá / Lập báo cáo / Chờ phê duyệt / Hoàn thành / Hủy). SRS `srs-fr-08-danh-gia.md:73,1052` confirm SM v3.5 8 states không có DA_DANH_GIA; transition THUC_HIEN→BAO_CAO là AUTO theo FR-VI-06 Bước 8 ("tất cả VV trong đợt đã đánh giá? Nếu có → chuyển BAO_CAO"). KHÔNG cần endpoint forward POST — QA dùng state machine v3 CŨ là bug giả. Đóng Closed-Invalid.

### Mô tả

Account `cb_nv_tw_02` (owner) đợt `DG-20260513-0001` đã được advance qua state machine: `LAP_KE_HOACH → PHAN_CONG → CHO_DUYET_PC → THUC_HIEN` thành công qua UI button click (POST `/phan-congs` 201 → auto-advance, POST `/phan-congs/submit` 200, POST PD approve 200). Tới state `THUC_HIEN` ver 4, không có button UI nào tiếp tục advance forward, và mọi endpoint variant POST đều trả 404. Hậu quả: 3 state cuối lifecycle (`DANG_DANH_GIA`, `DA_DANH_GIA`, `HOAN_THANH`) không thể đạt qua workflow API từ state `THUC_HIEN`. SM-DANHGIA spec (`srs-fr-08-danh-gia.md` line 1133-1180) yêu cầu các transition `THUC_HIEN → DANG_DANH_GIA` (khi bắt đầu chấm điểm), `DANG_DANH_GIA → DA_DANH_GIA` (khi đã đủ ket-qua), `DA_DANH_GIA → HOAN_THANH` (khi Trình kết quả). Đợt seed pre-existing `aaff0010-0000-4000-8000-000000000001` ở state `HOAN_THANH` chứng tỏ trước đó có cơ chế khác (chưa rõ — có thể seed DB direct, không qua workflow API).

### Các bước tái hiện

**Precondition:** Đợt đánh giá ở state `THUC_HIEN` (đã qua phê duyệt PC). Account owner role `CB_NV_TW` (vd `cb_nv_tw_02` / `cb_nv_tw_03`).

1. Login `cb_nv_tw_02` → tạo đợt mới `DG-20260513-0001` → add 2 tiêu chí Σ=100% → add 2 PC → state auto `PHAN_CONG` ver 2.
2. Click [Trình phê duyệt] → confirm → state `CHO_DUYET_PC` ver 3.
3. Login `cb_pd_tw_02` → mở đợt → click [Phê duyệt] → confirm → state `THUC_HIEN` ver 4.
4. Re-login `cb_nv_tw_03` (Trưởng nhóm PC) → mở đợt tab "Chấm điểm" → [Lưu kết quả] button visible nhưng 0 VV (do đợt mới chưa có VV nào).
5. Click [Lưu kết quả] → FE thử POST các endpoint variant: `submit-ket-quas`, `hoan-thanh-cham-diem`, `cham-diem/submit`, `ket-quas/submit`, `tong-hop`, `tong-hop-ket-qua` → **TẤT CẢ trả 404 ERR-SYS-00-04-01** "Cannot POST /api/v1/...".
6. Curl thêm các endpoint variant cho transition tiếp theo: `finish`, `hoan-thanh`, `gui-bao-cao`, `submit-bao-cao`, `ket-thuc`, `bao-cao`, `start-cham-diem`, `bat-dau-cham-diem`, `thuc-hien/submit`, `mo-cham-diem` → **TẤT CẢ trả 404**.

### Kết quả mong đợi

Theo SRS `srs-update-2026-5-5/srs-fr-08-danh-gia.md` State Machine line 1133-1180:
- Transition `THUC_HIEN → DANG_DANH_GIA`: trigger khi cán bộ NV bắt đầu chấm điểm (ĐGV mở Tab Thực hiện và submit lần đầu).
- Transition `DANG_DANH_GIA → DA_DANH_GIA`: trigger khi tất cả VV trong đợt có ket-qua đủ → Trưởng nhóm hoặc CB NV "Trình kết quả".
- Transition `DA_DANH_GIA → HOAN_THANH`: trigger khi CB PD phê duyệt báo cáo kết quả.

Mỗi transition cần endpoint REST POST tương ứng (vd `/phan-congs/submit` cho `PHAN_CONG → CHO_DUYET_PC`, theo pattern đã có). FE/UI trigger qua button "Lưu kết quả" / "Trình kết quả" / "Phê duyệt báo cáo".

### Kết quả thực tế

Mọi POST endpoint variant cho 3 transition forward đều trả `404 ERR-SYS-00-04-01 "Cannot POST /api/v1/..."`. UI button "Lưu kết quả" trigger 6 endpoint variant đồng loạt — tất cả 404 (network log evidence kèm). Đợt seed `DG-20260513-0001` stuck ở `THUC_HIEN` ver 4 sau workflow advance. Đợt pre-existing `aaff0010-0000-4000-8000-000000000001` ở state HOAN_THANH với `diemTrungBinh: 8.25` chứng tỏ có cơ chế khác (DB seed direct) đã advance qua các state này — không qua workflow API. Workflow E2E từ tạo đợt đến HOAN_THANH bị block tại bước `THUC_HIEN → DANG_DANH_GIA` trở đi.

### Bằng chứng

**Network log (curl POST + FE button click `[Lưu kết quả]`):**
```
POST /api/v1/ke-hoach-danh-gias/440b6dd1-d086-41d6-a842-45d2a323c94a/submit-ket-quas        → 404
POST /api/v1/ke-hoach-danh-gias/440b6dd1-d086-41d6-a842-45d2a323c94a/hoan-thanh-cham-diem  → 404
POST /api/v1/ke-hoach-danh-gias/440b6dd1-d086-41d6-a842-45d2a323c94a/cham-diem/submit       → 404
POST /api/v1/ke-hoach-danh-gias/440b6dd1-d086-41d6-a842-45d2a323c94a/ket-quas/submit        → 404
POST /api/v1/ke-hoach-danh-gias/440b6dd1-d086-41d6-a842-45d2a323c94a/tong-hop               → 404
POST /api/v1/ke-hoach-danh-gias/440b6dd1-d086-41d6-a842-45d2a323c94a/tong-hop-ket-qua       → 404
```

**Response body sample:**
```json
{"success":false,"error":{"code":"ERR-SYS-00-04-01","message":"Cannot POST /api/v1/ke-hoach-danh-gias/440b6dd1-d086-41d6-a842-45d2a323c94a/submit-ket-quas","timestamp":"2026-05-12T17:..."}}
```

**State trước-sau:** GET đợt `DG-20260513-0001` trước = `{state: "THUC_HIEN", version: 4}` → sau 14 attempt POST forward = `{state: "THUC_HIEN", version: 4}` (KHÔNG đổi).

**Đợt seed direct (đối chứng):** GET `aaff0010-0000-4000-8000-000000000001` = `{state: "HOAN_THANH", version: 1, diemTrungBinh: 8.25, soVuViecDanhGia: 12}` — state này không thể đạt qua workflow API hiện tại từ THUC_HIEN.

![BUG-DG-016 chấm điểm tab block no endpoint](image/r19c-seed-step9-cham-diem-block-no-vv-013700.png)
...
## ~~BUG-FUNC-DG-014~~ [CLOSED] — Dropdown Lĩnh vực modal "Thêm người đánh giá" render 2 raw UUID thay vì tên Vietnamese

> **Re-test:** 2026-05-12 22:40:00 R20 — ✅ PASS (Closed-verified). Login `cb_nv_tw_02` → đợt `DG-20260511-0001` PHAN_CONG → Tab Phân công → [+ Thêm người đánh giá] mở modal → click dropdown "Lĩnh vực" → 10 options render đầy đủ tên Vietnamese (Thuế / Lao động / Đất đai / Dân sự / Thương mại / Hình sự / Hành chính / Sở hữu trí tuệ / Doanh nghiệp / Đầu tư), KHÔNG còn UUID raw nào. API `GET /api/v1/danh-muc?loaiDanhMuc=LINH_VUC_PL&pageSize=100` trả 10 record cùng `ten` Vietnamese. BA/dev đã loại 2 record `bbbbbbbb-...-0018` + `bbbbbbbb-...-0013` UUID-only (R19 → R20 còn 10). Đóng bug Medium.
