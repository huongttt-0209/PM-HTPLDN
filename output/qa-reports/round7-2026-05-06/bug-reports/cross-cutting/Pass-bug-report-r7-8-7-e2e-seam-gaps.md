...
Phát hiện **3 lỗi có SRS reference cụ thể** khi run E2E 12 bước DN + Re-verify S5/S4 (2026-05-13 11:00:00). Hiện trạng sau R20 reverify: **3 Closed**. BUG-E2E-S4 ✅: DN portal nay có button **"Gửi yêu cầu hỗ trợ pháp lý"** + modal mở khi click — UC52 đã triển khai FE side. BUG-E2E-S5 ✅: Accordion 5 nay render label "Đơn vị quản lý" + value "Cục Bổ trợ tư pháp - Bộ Tư pháp" (KHÔNG còn "Địa bàn" + address). BUG-E2E-S6 Closed từ R19.
...
|---|---|---|---|---|---|---|---|
| ~~BUG-E2E-S4~~ | **Critical** | P0 | Workflow | R7.8.7-S4 | `srs-update-2026-5-5/srs-v3.5.md SCR-V.I-04 + FR-V.I-02 UC52` | ~~DN không có CTA gửi yêu cầu HTPL — UC52 chưa triển khai (app tự khai báo qua toast)~~ | Closed ✅ R20 |
| ~~BUG-E2E-S5~~ | Minor | P2 | UI/UX | R7.8.7-S5 | `srs-fr-05-vu-viec.md` line 18 (v3.5 rev. 2 G2+G4) + FR-V.I-09 Outputs.6 (`don_vi_quan_ly`) | ~~SCR-V.I-03 Accordion 5 hiển thị label "Địa bàn" + value address thay vì "Đơn vị quản lý" + Sở TP/Bộ ngành~~ | Closed ✅ R20 |
...
## ~~BUG-E2E-S4~~ [CLOSED] — DN không có CTA gửi yêu cầu HTPL, UC52 chưa triển khai

> **Re-test:** 2026-05-13 11:00:00 R20 — ✅ PASS (Closed-verified). Login DN `9999999990` qua UI (fill_form + submit OTP 666666) → landing dashboard với sidebar 4 menu DN (Tổng quan / Quản lý đào tạo / Quản lý vụ việc HTPL / Quản lý doanh nghiệp) → navigate `/vu-viec/danh-sach` → render **button "Gửi yêu cầu hỗ trợ pháp lý"** (icon `send`) cùng với button "Làm mới". Click button → **modal "Gửi yêu cầu hỗ trợ pháp lý" mở** (`.ant-modal` visible với title đúng). UC52 đã triển khai FE side. Evidence: [`image/r20-bug-e2e-s4-dn-cta-uc52-pass.png`](image/r20-bug-e2e-s4-dn-cta-uc52-pass.png).
...
## ~~BUG-E2E-S5~~ [CLOSED] — SCR-V.I-03 Accordion 5 vẫn label "Địa bàn" + value address (chưa apply v3.5 rev. 2 G2+G4)

> **Re-test:** 2026-05-13 11:00:00 R20 — ✅ PASS (Closed-verified). Login `cb_nv_tw_05` → navigate VV `VV-BTP-TW-20260511-003` (id `cdeb01f4-0f80-4b7a-ac1f-7f439c9bcd32`, state DA_PHAN_CONG) → expand Accordion 5 "Phân công Người hỗ trợ / Tư vấn viên". Render: label **"Đơn vị quản lý"** (uid=210_2) + value **"Cục Bổ trợ tư pháp - Bộ Tư pháp"** (uid=210_3) — đúng spec v3.5 rev. 2 G2+G4. KHÔNG còn label "Địa bàn" + address. FE đã apply rename. Evidence: [`image/r20-bug-e2e-s5-donvi-quanly-pass.png`](image/r20-bug-e2e-s5-donvi-quanly-pass.png).
