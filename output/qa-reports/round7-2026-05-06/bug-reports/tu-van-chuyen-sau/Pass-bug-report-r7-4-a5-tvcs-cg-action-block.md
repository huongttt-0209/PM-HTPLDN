...
| **Ngày** | 2026-05-07 21:54:00 (R8 log) · 2026-05-09 20:25:00 (R9 re-test) · 2026-05-09 23:50:00 (R10 re-test bộ TK 06) · 2026-05-10 09:25:00 (R11 verify dev fix) · 2026-05-10 09:40:57 (R12 verify dev fix BUG-004) · 2026-05-10 12:10:32 (R13 verify dev fix BUG-004 lần 2) · 2026-05-10 13:25:00 (R14 verify dev fix lần 3 + bộ acc `_07`) · **2026-05-10 14:50:00 (R15 verify BE-fix lần 4 — commit `f54afbc8` "CG fill ket_qua atomically khi complete TVCS")** |
| **Loại test** | Workflow (R7.4.A5) |
| **Round** | R8 → R9 → R10 → R11 → R12 → R13 → R14 → **R15** (verify BE-fix `f54afbc8` — 2026-05-10 14:50:00, BUG-A5-004 **CLOSED-verified**) |
...
**R15 status (2026-05-10 14:50:00):** **4/4 đóng** ✅ BUG-FE-A5-004 **CLOSED-verified.** BE deploy commit `f54afbc8` "CG fill ket_qua atomically khi complete TVCS" — POST `/hoan-thanh {version, ketQua, ghiChu?}` giờ 200 trong ~250ms, persist `ketQua` vào DB và transition state DANG_TU_VAN → CHO_PHE_DUYET ver+1 atomically (option C SRS line 1294 atomic save+complete). Verify trên TVCS-20260507-0013 (Thuế, huongcg phân công) — UI walk: huongcg login → click row → click [Hoàn thành] → modal "Hoàn thành tư vấn" có textarea `Kết quả *` (224 chars) + textarea `Ghi chú` → submit → POST 200 → state CHO_PHE_DUYET ver=3. Sau đó cb_pd_tw_06 click [Phê duyệt] → POST `/duyet` 200 → state DA_DUYET ver=4 (B8 PASS). Reject path verify trên TVCS-20260507-0013 cycle 2: huongcg HOAN_THANH (134 chars) → CHO_PHE_DUYET ver=4 → cb_pd_tw_06 click [Từ chối] modal "Từ chối nội dung tư vấn" textarea `Lý do từ chối` 198 chars → POST `/reject` 200 → state DANG_TU_VAN ver=5, ketQua preserved (134 chars) (B9 PASS). Workflow R7.4.A5 R15: **9/11 PASS + 2 EXTERNAL = 11/11 covered** (vs R14 7/11 PASS).
...
|--------|----------|----------|------|--------|-------------------|-------|--------|
| ~~BUG-FE-TVCS-A5-004~~ | Critical | P0 | UI/UX + Workflow | TC-A5-B6 (HOAN_THANH) | `srs-fr-12-tv-chuyen-sau.md` line 189 (`Kiểm tra ket_qua không rỗng`) + line 1292 (`ket_qua text long Kết quả tư vấn (VB TVPL)`) | ~~UI/API gap — CG không có cách fill `ket_qua` trong DANG_TU_VAN. R15 verify commit `f54afbc8`: POST `/hoan-thanh {version, ketQua, ghiChu}` giờ 200 atomic save+complete (option C). B6 PASS UI walk huongcg → modal có textarea `Kết quả *` 224 chars → submit 200 → state CHO_PHE_DUYET. B8 + B9 cascade PASS.~~ | Closed |
...
## ~~BUG-FE-TVCS-A5-004~~ [CLOSED] — UI/API gap: CG không có cách fill `ket_qua` trong DANG_TU_VAN, block B6 → cascade

> **Re-test:** 2026-05-10 14:50:00 R15 — ✅ **PASS (Closed-verified).** BE deploy commit `f54afbc8` "CG fill ket_qua atomically khi complete TVCS" (option C atomic save+complete per SRS line 1294). UI walk full B6 → B7 → B8 + B9 cycles trên TVCS-20260507-0013 (Thuế, huongcg phân công). Setup: huongcg accept POST `/xac-nhan {CHAP_NHAN, version:2}` 200 ver=3 DANG_TU_VAN. **B6 PASS:** huongcg click [Hoàn thành] → modal "Hoàn thành tư vấn" có textarea `Kết quả *` + `Ghi chú` → fill 224 chars (counter `224/50000`) → submit → POST `/hoan-thanh {version:3, ketQua, ghiChu}` **HTTP 200 in ~250ms**. State DANG_TU_VAN → CHO_PHE_DUYET ver=4 atomically. **B7 PASS** auto-trigger. **B8 PASS:** cb_pd_tw_06 click [Phê duyệt] → POST `/duyet {version:4}` 200 → state DA_DUYET ver=5. **B9 PASS reject path** (cycle 2 trên TVCS-0013): huongcg HOAN_THANH 134 chars → CHO_PHE_DUYET ver=4 → cb_pd_tw_06 click [Từ chối] modal "Từ chối nội dung tư vấn" textarea `Lý do từ chối` 198 chars → POST `/reject` 200 → state CHO_PHE_DUYET → DANG_TU_VAN ver=5 rollback, ketQua preserved 134 chars (NOT cleared on reject). **Verdict R15:** BUG-FE-A5-004 fully closed — FE form complete + BE atomic endpoint accept ket_qua + cb_pd_tw approve/reject cascade work end-to-end. Workflow R7.4.A5 R15 **9/11 PASS + 2 EXTERNAL = 11/11 covered** (B5 cron-EXT, B11 Portal DN-EXT). Bằng chứng R15: [`r7-4-a5-r15-b6-PASS-cho-phe-duyet.png`](image/r7-4-a5-r15-b6-PASS-cho-phe-duyet.png), [`r7-4-a5-r15-b8-PASS-da-duyet.png`](../../workflow/tu-van-chuyen-sau/image/r7-4-a5-r15-b8-PASS-da-duyet.png), [`r7-4-a5-r15-b9-PASS-tu-choi-rollback.png`](../../workflow/tu-van-chuyen-sau/image/r7-4-a5-r15-b9-PASS-tu-choi-rollback.png).
>
