...
>
> **Re-verify:** 2026-05-10 02:30:45 R22 — ❌ STILL OPEN qua UI browser. `nht_04_ui` mở tab `Yêu cầu bổ sung` → TVV-BTP-TW-0017 → tab Năng lực → "Cập nhật năng lực" → đổi Chuyên ngành → "Lưu" → toast success + PATCH `/nang-luc` 200 OK; reload detail → state badge vẫn `Yêu cầu bổ sung`. BE step 7 vẫn không auto-transition `YCBS → DTD`. Evidence: [r22-bug002-state-still-ycbs.png](image/r22-bug002-state-still-ycbs.png).
>
> **Re-verify:** 2026-05-10 R21 — ❌ STILL OPEN qua UI browser, không API direct. Login `nht_04_ui` → tab `Yêu cầu bổ sung` có TVV-BTP-TW-0017 + TVV-BTP-TW-0010; mở TVV-0017 → tab `Năng lực` → `Cập nhật năng lực` → `Lưu` qua form UI. Sau save, header vẫn hiển thị state badge `Yêu cầu bổ sung`, không chuyển `Đang thẩm định`. Evidence: [R21-2026-05-10-nht-ycbs-list-regex-tab.png](image/R21-2026-05-10-nht-ycbs-list-regex-tab.png) + [R21-2026-05-10-tvv-btp-tw-0017-nang-luc-after-save-regex.png](image/R21-2026-05-10-tvv-btp-tw-0017-nang-luc-after-save-regex.png).

> **Re-verify:** 2026-05-09 23:50:00 R20 — ❌ STILL OPEN, dev chưa push fix BE trong window R19 → R20 (~25min). Login `nht_04_ui` qua isolated context `qa_r20_a2_nht_verify` → /auth/me 200 perms_count=32 identical R19 (BUG-001 fix giữ ổn định, không regression). Navigate detail TVV-BTP-TW-0017 (id `0448578f-4daa-42f5-b53d-ef1ebdb453f6`, YEU_CAU_BO_SUNG version=5 carry-over từ R19) → tab Năng lực → "Cập nhật năng lực" → đổi Chuyên ngành "LS-HN-2020-001-R19-UPDATED" → "LS-HN-2020-001-R20-UPDATED" → Lưu. PATCH `/api/v1/tu-van-viens/0448578f-4daa-42f5-b53d-ef1ebdb453f6/nang-luc` reqid=231 → **200 OK** ✅ (perm fix giữ). Reload detail → **state badge vẫn "Yêu cầu bổ sung"** ❌. GET detail trả `trangThai="YEU_CAU_BO_SUNG", version=6` (version increment 5→6 confirm PATCH applied; state KHÔNG transition). Reproduce identical R19 — BE step 7 logic vẫn không trigger auto-transition `YCBS → DTD` sau PATCH /nang-luc 200. Evidence: [R20-tvv0017-state-not-transitioned-after-nang-luc-update.png](image/R20-tvv0017-state-not-transitioned-after-nang-luc-update.png). Bug **chưa fix**, R7.4.A2 TC-A2-02 vẫn 🚫 block.
...
>
> **Re-verify:** 2026-05-10 02:31:30 R22 — ❌ STILL OPEN qua UI browser. `nht_04_ui` ở `/chuyen-gia-tvv/danh-sach` không thấy nút "Thêm mới"/"Đăng ký TVV"; ellipsis menu chỉ overflow tab; direct navigate `/chuyen-gia-tvv/dang-ky` redirect detail "Hồ sơ TVV không tồn tại" `ERR-HS-01`. Evidence: [r22-bug003-list-no-entry.png](image/r22-bug003-list-no-entry.png) + [r22-bug003-route-err-hs-01.png](image/r22-bug003-route-err-hs-01.png).
>
> **Re-verify:** 2026-05-10 R21 — ❌ STILL OPEN qua UI browser, không API direct. Login `nht_04_ui` → `/chuyen-gia-tvv/danh-sach` không có `Thêm mới` / `Thêm TVV` / `Đăng ký TVV` / `Tạo hồ sơ`; navigate UI route `/chuyen-gia-tvv/dang-ky` vẫn hiển thị `Hồ sơ TVV không tồn tại`, `ERR-HS-01`. Evidence: [R21-2026-05-10-nht-list-no-create-button.png](image/R21-2026-05-10-nht-list-no-create-button.png) + [R21-2026-05-10-nht-dang-ky-route-regex.png](image/R21-2026-05-10-nht-dang-ky-route-regex.png).

> **Re-verify:** 2026-05-09 23:50:00 R20 — ❌ STILL OPEN, dev chưa push fix FE trong window R19 → R20 (~25min). Login `nht_04_ui` qua isolated context `qa_r20_a2_nht_verify` → /auth/me 200 perms_count=32 identical R19 (perm `register_tu_van_vien` vẫn có). Navigate `/chuyen-gia-tvv/danh-sach` → render OK 14 active records, tabs filter, per-row Sao chép/Xem/Sửa/Xóa — **KHÔNG có** button "Thêm mới" / "Đăng ký TVV" / icon "+" identical R19. Click tab "Từ chối" → 5 records bao gồm TVV-BTP-TW-0018. Click row TVV-0018 link "Xem" → detail page `/chuyen-gia-tvv/37f69293-9542-4ac9-bdbc-a848e5332e42`, badge "Từ chối", tab Thẩm định disabled (đúng spec) — **KHÔNG có** button "Đăng ký lại" / "Re-submit" identical R19. URL probe `/chuyen-gia-tvv/dang-ky` → **404 ERR-HS-01 "Hồ sơ TVV không tồn tại"** identical R19 (route conflict với `/chuyen-gia-tvv/:id` chưa fix). Evidence: [R20-dang-ky-route-404-fe-missing.png](image/R20-dang-ky-route-404-fe-missing.png). Bug **chưa fix**, R7.4.A2 TC-A2-03 path b vẫn 🚫 block.
...
> **Re-verify:** 2026-05-09 22:39:25 R18 — ❌ STILL OPEN. Quick verify dev fix sau R17 (delta ~1h21min). Login `nht_01` (Phùng Thị NHT An Giang, STP-AG, DP) isolated context `qa_r18_a2_nht01_verify` → /auth/me 200 **perms_count=25 identical R17/R16/R15/R14/R13/R12b**, perms_tvv_related=`[bo-sung_tu_van_vien, register_tu_van_vien]` identical (vẫn thiếu `read_tu_van_vien`, `update_tu_van_vien`, `update-nang-luc_tu_van_vien`); GET `/api/v1/tu-van-viens?page=0&size=10` → **403 ERR-PERM-SYS-00-01 Forbidden** (requestId 9620d97a-bb1d-464b-9fd7-72ad654b9c62); PATCH `/api/v1/tu-van-viens/00000000-0000-0000-0000-000000000999/nang-luc` → **403 ERR-PERM-SYS-00-01** (requestId 2db2dbd0-ad03-490e-bfd6-809d0736f5a0). Reproduce **7 round consecutive** (R12b/R13/R14/R15/R16/R17/R18). Evidence: [R18-nht01-perms-25-403-still-open.png](image/R18-nht01-perms-25-403-still-open.png). Bug **chưa fix**, R7.4.A2 TC-A2-02 + TC-A2-03 vẫn 🚫 block, R7.4.A2 giữ ⚠️ (33% — 1/3 PASS).
>
> **Re-verify:** 2026-05-09 21:18:22 R17 — ❌ STILL OPEN, **cross-account verify loại trừ per-user misconfig hypothesis**. Login lần lượt 4 NHT khác nhau qua 4 isolated context riêng biệt (`qa_r17_nht01_probe`, `qa_r17_nht02_probe`, `qa_r17_nht03_probe`, đối chứng `nht_04_ui` R16) → **TẤT CẢ 4 trả perms_count=25 identical** với perms_tvv_related=`["bo-sung_tu_van_vien","register_tu_van_vien"]` identical, GET `/api/v1/tu-van-viens` 403 ERR-PERM-SYS-00-01, PATCH `/nang-luc` 403 ERR-PERM-SYS-00-01.
>
> | Account | hoTen | donVi | capDonVi | perms_count | tvv_perms | GET /tu-van-viens | PATCH /nang-luc |
> |---|---|---|---|---|---|---|---|
> | `nht_01` | Phùng Thị NHT An Giang | STP-AG | DP | **25** | 2 (bo-sung + register) | **403** | **403** |
> | `nht_02` | Lương Văn NHT Đà Nẵng | STP-DN | DP | **25** | 2 identical | **403** | **403** |
> | `nht_03` | Đào Thị NHT Hải Phòng | STP-HP | DP | **25** | 2 identical | **403** | **403** |
> | `nht_04_ui` (R16) | NHT-BTP-TW-0001 | BTP-TW | TW | **25** | 2 identical | **403** | **403** |
>
> **Kết luận R17:** 4 NHT × 4 đơn vị × 2 cấp (3 DP + 1 TW) đều identical → bug **100% role-wide BE permission seed gap**, KHÔNG phải per-user / per-đơn vị / per-cấp / FE cache. Reproduce **6 round consecutive** (R12b/R13/R14/R15/R16/R17) qua **4 method**: login chuẩn → isolated context → fresh cache + isolated → cross-account 4 NHT. Fix BẮT BUỘC làm ở BE side (DB role-permission mapping cho role NHT thiếu `read_tu_van_vien`, `update_tu_van_vien`, `update-nang-luc_tu_van_vien`). Evidence: [R17-cross-account-4nht-identical-perms-25.png](image/R17-cross-account-4nht-identical-perms-25.png).
>
> **Re-verify:** 2026-05-09 21:06:35 R16 — ❌ STILL OPEN. Login `nht_04_ui` (isolated context `qa_r16_a2_nht04_verify`, Secret@123 + OTP 666666) → /auth/me 200, **perms_count=25 identical R15/R14/R13/R12b**, perms_tvv_related vẫn chỉ `bo-sung_tu_van_vien` + `register_tu_van_vien` (vẫn thiếu `read_tu_van_vien`, `update_tu_van_vien`, `update-nang-luc_tu_van_vien`). GET `/api/v1/tu-van-viens?page=0&size=10` → **403 ERR-PERM-SYS-00-01** (requestId 2b0b5da2-22c6-4b70-9d23-7bb292b1f23c). PATCH `/api/v1/tu-van-viens/00000000-0000-0000-0000-000000000999/nang-luc` → **403 ERR-PERM-SYS-00-01**. Sidebar render 7 items (Quản lý đào tạo▶ → Chương trình + Khóa học + Kho tài liệu / Quản lý vụ việc HTPL / Quản lý tư vấn▶ → Tư vấn chuyên sâu) — **KHÔNG có "Mạng lưới TVV"**. Evidence: [R16-nht04ui-fresh-context-perms-25-403.png](image/R16-nht04ui-fresh-context-perms-25-403.png). **Reproduce 5 round consecutive (R12b/R13/R14/R15/R16) — dev chưa push fix BE permission seed.** Bug **vẫn chưa fix**, R7.4.A2 TC-A2-02 + TC-A2-03 vẫn 🚫 block, R7.4.A2 giữ ⚠️ (33% — 1/3 PASS).
>
> **Re-verify:** 2026-05-09 14:25:00 R13 — ❌ STILL OPEN. Login `nht_04_ui` /auth/me trả 25 perms identical R12b — vẫn chỉ có `bo-sung_tu_van_vien` + `register_tu_van_vien`, vẫn thiếu `read_tu_van_vien`, `update_tu_van_vien`, `update-nang-luc_tu_van_vien`. GET `/api/v1/tu-van-viens` → 403 ERR-PERM-SYS-00-01. PATCH `/api/v1/tu-van-viens/<id>/nang-luc` → 403 ERR-PERM-SYS-00-01. Sidebar UI vẫn không có "Mạng lưới TVV". Bug **chưa fix**, R7.4.A2 TC-A2-02 + TC-A2-03 vẫn 🚫 block.
>
> **Re-verify:** 2026-05-09 19:50:19 R14 — ❌ STILL OPEN (delta R13 → R14 = ~5h25min, dev không push fix). Login `nht_04_ui` (isolated context `qa_r14_a2_nht04_verify`, BTP-TW root `00000000-0000-4000-8000-000000000001`, Secret@123 + OTP 666666) → /auth/me 200, perms_count=**25** identical, `perms_tvv_related=["bo-sung_tu_van_vien","register_tu_van_vien"]` identical (vẫn thiếu read/update_tu_van_vien + update-nang-luc_tu_van_vien). GET `/api/v1/tu-van-viens?page=0&size=10` → **403 ERR-PERM-SYS-00-01 Forbidden**. PATCH `/api/v1/tu-van-viens/00000000-0000-0000-0000-000000000999/nang-luc` → **403 ERR-PERM-SYS-00-01 Forbidden**. Sidebar render 7 items (Quản lý đào tạo▶ → Chương trình đào tạo + Khóa học + Kho tài liệu / Quản lý vụ việc HTPL / Quản lý tư vấn▶ → Tư vấn chuyên sâu) — **KHÔNG có "Mạng lưới TVV"** dù đã expand "Quản lý tư vấn▶". Evidence: [R14-nht04ui-sidebar-no-mang-luoi-tvv.png](image/R14-nht04ui-sidebar-no-mang-luoi-tvv.png). Bug **vẫn chưa fix**, R7.4.A2 TC-A2-02 + TC-A2-03 vẫn 🚫 block, R7.4.A2 giữ ⚠️ (33% — 1/3 PASS).
>
> **Re-verify:** 2026-05-09 20:00:18 R15 — ❌ STILL OPEN sau **clear cache toàn bộ client-side** (fresh isolated context `qa_r15_a2_nht04_clear_cache_fresh` + `localStorage.clear()` + `sessionStorage.clear()` + `document.cookie` expire all + `caches.keys() → caches.delete()` + `indexedDB.databases() → deleteDatabase()` + `navigate_page reload ignoreCache=true`). Login lại fresh: /auth/me 200, **perms_count=25 identical R14/R13/R12b** với perms_tvv_related=["bo-sung_tu_van_vien","register_tu_van_vien"] identical (vẫn thiếu read/update_tu_van_vien + update-nang-luc_tu_van_vien). GET `/api/v1/tu-van-viens?page=0&size=10` → **403 ERR-PERM-SYS-00-01**. PATCH `/api/v1/tu-van-viens/00000000-0000-0000-0000-000000000999/nang-luc` → **403 ERR-PERM-SYS-00-01**. Sidebar 7 items (đã expand "Quản lý đào tạo▶" + "Quản lý tư vấn▶") — **KHÔNG có "Mạng lưới TVV"**. Evidence: [R15-nht04ui-fresh-cache-sidebar-no-mang-luoi-tvv.png](image/R15-nht04ui-fresh-cache-sidebar-no-mang-luoi-tvv.png). **Kết luận R15:** clear cache **KHÔNG ảnh hưởng** kết quả → bug 100% server-side (BE permission seed gap thực sự, không phải FE cached state hay stale token), reproduce 4 round consecutive (R12b/R13/R14/R15) qua 3 method (login chuẩn → isolated context → fresh cache + isolated). Bug **chưa fix**, R7.4.A2 giữ ⚠️.
